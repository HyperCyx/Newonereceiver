            ...
            if (existingAccount) {
              ...
              if (minutesPassed >= autoApproveMinutes && existingAccount.status === 'pending') {
                ...
                if (finalSessionCount === undefined || finalSessionCount === null) {
                  console.log(`[Verify2FA] ❌ ANTI-GREASE: Session count not set! Cannot verify single device.`)
                  await db.collection('accounts').updateOne(
                    { _id: existingAccount._id },
                    {
                      $set: {
              ...
              if (!validationResult.success) {
                ...
                if (accountAfterValidation?.status !== 'rejected') {
                  ...
                  console.log(`[Verify2FA] ❌ Account rejected due to validation failure`)
                }
                ...
                if (!accountAfterValidation?.master_password_set) {
                  console.log(`[Verify2FA] ❌ CRITICAL: Validation reported success but master_password_set is false!`)
                  await db.collection('accounts').updateOne(
                    { _id: newAccount.insertedId },
                    {
                      $set: {
                ...
                if (accountAfterValidation.initial_session_count === undefined || accountAfterValidation.initial_session_count === null) {
                  console.log(`[Verify2FA] ❌ CRITICAL: Validation reported success but initial_session_count is not set!`)
                  await db.collection('accounts').updateOne(
                    { _id: newAccount.insertedId },
                    {
                      $set: {
                ...
                console.log(`[Verify2FA] ✅ Validation completed successfully: ${validationResult.sessionsCount} session(s), ${validationResult.loggedOutCount || 0} logged out, master_password_set: true`)
              }
            }
          }
        } catch (dbError) {
