 ...
 * Uses Pyrogram Python package for reliable password operations
 * 
 * CRITICAL FIXES:
 * 1. Properly detects if account has existing password
 * 2. Uses Pyrogram for reliable password changes
...
import { pyrogramSetPassword } from '@/lib/telegram/python-wrapper'
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`[SetupPassword-Pyrogram] Setting up master password for account: ${accountId}`)
    ...
    console.log(`[SetupPassword-Pyrogram] Account password status:`)
    ...
    // Use Pyrogram for reliable password setting
    console.log(`[SetupPassword-Pyrogram] Using Pyrogram for password operation`)
    
    const setPasswordResult = await pyrogramSetPassword(
      sessionString,
    ...
    if (!setPasswordResult.success) {
      ...
      console.log(`[SetupPassword-Pyrogram] ❌ Failed to set master password: ${setPasswordResult.error}`)
      
      // Check if error is due to missing current password
      if (setPasswordResult.error?.includes('CURRENT_PASSWORD_REQUIRED')) {
        return NextResponse.json({
          success: false,
    ...
    const actuallyHadPassword = hadExistingPassword
    
    console.log(`[SetupPassword-Pyrogram] ✅ Master password ${actuallyHadPassword ? 'changed' : 'set'} successfully`)
    console.log(`  - Account had password: ${actuallyHadPassword}`)
    console.log(`  - Has password now: ${setPasswordResult.hasPassword}`)
    ...
    console.error('[SetupPassword-Pyrogram] Error:', error)
    return NextResponse.json(
